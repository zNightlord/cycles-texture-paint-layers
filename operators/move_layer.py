...
class LAYER_OT_movelayer(bpy.types.Operator):
    ...
    def move_nodes(self, index, neighbor):
        ...
        if higher is not None:
            ...
            tree.links.new(higher.outputs[0], to_higher_node.inputs[col_input])
        col_input = layer_utils.get_input_of_type(lower, 'RGBA')
        tree.links.new(to_lower_node.outputs[0], lower.inputs[col_input])
        tree.links.new(to_higher_node.outputs[0], to_lower_node.inputs[1])
    ...
    def link_add_nodes(self, low_index, high_index, higher):
        ...
        tree.links.new(low_index_node.outputs[0], low_out)
        tree.links.new(high_index_node.outputs[0], high_out)
        if higher is not None:
            tree.links.new(higher.outputs[0], high_index_node.inputs[0])
