...
class LAYER_OT_dellayer(bpy.types.Operator):
    ...
    def delete_nodes(self, context):
        ...
        if higher is not None:
            ...
            mat.node_tree.links.new(higher.outputs[0], lower.inputs[col_input])
        if higher_add is not None:
            add_out = layer_utils.get_alpha_output(context, index)
            mat.node_tree.links.new(higher_add.outputs[0], add_out)
